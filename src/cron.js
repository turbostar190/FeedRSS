...
/**
 * Ottiene gli uid degli utenti registrati nel db
 * @returns {Object}
 */
function getUserUids(){
    return db.prepare('SELECT uid FROM user').all();
}
...
(async () => {
    const feed = await parser.parseURL('https://www.tomshw.it/feed/');
...
<i>${html2json(item.content).child[1].text}</i>
&#x1f4c6; ${datum}`;
